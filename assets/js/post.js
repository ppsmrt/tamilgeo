...
const loader = document.querySelector(".dial-loader"); // spinner element
...
fetch(postURL)
  ...
  .then(post => {
    ...
    postElement.className = "bg-white p-6 rounded-lg shadow-md opacity-0 scale-95 transition-all duration-700";
    postElement.innerHTML = `
      ...
      <div class="flex gap-4 items-center mb-4 mt-6">
        <button id="like-btn" class="flex items-center gap-2 text-green-600"><i class="fa-regular fa-thumbs-up"></i> Like <span id="like-count">0</span></button>
        <button id="comment-btn" class="flex items-center gap-2 text-blue-600"><i class="fa-regular fa-comment"></i> Comment <span id="comment-count">0</span></button>
        <button id="share-btn" class="flex items-center gap-2 text-purple-600"><i class="fa-solid fa-share"></i> Share <span id="share-count">0</span></button>
      </div>
    ...
    // Append post after a short delay
    container.innerHTML = "";
    container.appendChild(postElement);

    // Animate post fade-in
    setTimeout(() => {
