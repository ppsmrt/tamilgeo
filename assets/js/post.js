import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-app.js"; import { getAuth, onAuthStateChanged } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js"; import { getDatabase, ref, push, onValue, update } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js";

// ✅ Firebase config const firebaseConfig = { apiKey: "AIzaSyDt86oFFa-h04TsfMWSFGe3UHw26WYoR-U", authDomain: "tamilgeoapp.firebaseapp.com", databaseURL: "https://tamilgeoapp-default-rtdb.firebaseio.com", projectId: "tamilgeoapp", storageBucket: "tamilgeoapp.appspot.com", messagingSenderId: "1092623024431", appId: "1:1092623024431:web:ea455dd68a9fcf480be1da" };

// ✅ Initialize Firebase const app = initializeApp(firebaseConfig); const auth = getAuth(app); const db = getDatabase(app);

// ✅ Get post ID from URL const postId = new URLSearchParams(window.location.search).get("id"); const container = document.getElementById("post-container");

if (!postId) { container.innerHTML = "<p class='text-red-500 text-center'>Invalid Post ID.</p>"; throw new Error("Missing post ID"); }

// ✅ WordPress API URL (supports numeric ID and slug) const isNumeric = !isNaN(postId); const postURL = isNumeric ? https://public-api.wordpress.com/wp/v2/sites/tamilgeo.wordpress.com/posts/${postId} : https://public-api.wordpress.com/wp/v2/sites/tamilgeo.wordpress.com/posts?slug=${postId};

let currentUser = null; // Track logged-in user

// ✅ Fetch post and render fetch(postURL) .then(res => { if (!res.ok) throw new Error(Failed to fetch post: ${res.status}); return res.json(); }) .then(async post => { const wpPost = Array.isArray(post) ? post[0] : post; if (!wpPost) { container.innerHTML = "<p class='text-red-600 font-semibold text-center'>Post not found.</p>"; return; }
...
let contentStyled = wpPost.content.rendered
  ...
  .replace(/<hr\s*\/?>(?![^<]*<\/code>)/g, `<div class="my-6 h-1 rounded-full bg-gradient-to-r from-green-400 via-green-600 to-green-400"></div>`)
  ...
  .replace(/<th>(.*?)<\/th>/g, '<th class="border border-green-600 text-black font-bold bg-orange-200 px-3 py-2">$1</th>')
...
if (wpPost.categories && wpPost.categories.length > 0) {
  const catRes = await fetch(`https://public-api.wordpress.com/wp/v2/sites/tamilgeo.wordpress.com/categories/${wpPost.categories[0]}`);
  if (catRes.ok) {
...
container.innerHTML = `
  <div class="w-full max-w-3xl px-4 py-6">
    <div class="bg-white p-6 rounded-3xl shadow-2xl hover:shadow-3xl transition-all duration-500 opacity-0" id="post-content-wrapper">
      ...
      <!-- Author Section -->
      <div class="flex items-center mb-4 bg-gray-50 p-3 rounded-xl">
        ...
        <span id="authorCategory" class="ml-auto text-sm px-3 py-1 rounded-full bg-gradient-to-r from-green-500 to-green-700 text-white font-medium shadow-md">${categoryName}</span>
      </div>

      <hr class="border-gray-300 my-6">

      <!-- React Section -->
      <div id="likeSection" class="pt-3 bg-gray-50 p-3 rounded-xl">
        <h3 class="text-gray-700 font-medium mb-2">React to this Post</h3>
        <div class="flex space-x-4 text-2xl" id="likeReactions">
          <button data-reaction="love" class="hover:scale-125 transition transform"><i class="fa-regular fa-heart text-red-500"></i></button>
          <button data-reaction="laugh" class="hover:scale-125 transition transform"><i class="fa-regular fa-face-laugh text-yellow-500"></i></button>
          <button data-reaction="wow" class="hover:scale-125 transition transform"><i class="fa-regular fa-face-surprise text-blue-500"></i></button>
          <button data-reaction="sad" class="hover:scale-125 transition transform"><i class="fa-regular fa-face-sad-tear text-indigo-500"></i></button>
          <button data-reaction="like" class="hover:scale-125 transition transform"><i class="fa-regular fa-thumbs-up text-green-600"></i></button>
        </div>
      ...
      <!-- Comment Box Section -->
      <div id="commentSection" class="mt-6 bg-gray-50 p-4 rounded-xl">
...
// ✅ Set Author Info (Admin/@tamilgeo for pulled posts)
document.getElementById("authorImage").src = wpPost.isPulled
  ? 'https://ppsmrt.github.io/tamilgeo/assets/icon/Logo.jpg'
  : (currentUser ? currentUser.photoURL : 'https://ui-avatars.com/api/?name=Author');

document.getElementById("authorName").textContent = wpPost.isPulled ? 'Admin' : (currentUser?.displayName || 'Author');
document.getElementById("authorUsernameEmail").textContent = wpPost.isPulled ? '@tamilgeo' : `@${currentUser?.email?.split("@")[0] || 'username'} • ${currentUser?.email || 'email@example.com'}`;
